...
2. The devcontainer CLI does attach a Podman event listener during startup.
   - An upstream devcontainers/cli issue log shows:
     - =podman run ... -c echo Container started=
     - then =podman events --format json --filter event=start=
   - This ordering matches the race signature we see locally.
   - Source: [[https://github.com/devcontainers/cli/issues/816][devcontainers/cli #816]]

3. VS Code Dev Containers logs with Podman also show event-listening as part of startup.
   - A vscode-remote-release issue log includes =podman events --filter event=start= after the container start step.
   - This confirms the event listener pattern is common in the official tooling path, not unique to jolo.
   - Source: [[https://github.com/microsoft/vscode-remote-release/issues/8157][vscode-remote-release #8157]]
