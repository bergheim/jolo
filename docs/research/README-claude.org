...
Jolo containers have no X11 socket and no ~DISPLAY~ variable. This prevents X11-based attacks (keylogging, screenshot capture, input injection). Wayland access is conditional — the socket is only mounted when ~WAYLAND_DISPLAY~ is set on the host. Wayland's per-surface isolation makes this safe (unlike X11).
