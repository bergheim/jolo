...
** TODO Auto-copy dev server URL to clipboard on =just dev=
   Prepend an OSC 52 clipboard copy line to every web justfile's =dev= recipe.
   When you type =just dev=, the URL (=http://localhost:$PORT=) goes to your
   clipboard before the server blocks. Paste in browser. No separate =browse=
   recipe needed — remove it from justfiles that have it.
