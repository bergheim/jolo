...
** TODO Fix =jolo delete= bare name resolution
   =jolo create foo= takes a bare name, but =jolo delete foo= only searches worktree
   names — can't find the project you just created. Fall back to =cwd / name= when no
   worktree matches, mirroring how =create= resolves names.
** TODO Wayland-aware =just browse= — open Chromium on host display
   ...
   Always copies URL to clipboard too. Falls back to clipboard-only without Wayland.
