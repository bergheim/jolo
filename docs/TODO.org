...
#+TODO: TODO NEXT INPROGRESS BLOCKED | DONE

* Tasks
** DONE Add Org TODO workflow automation for docs/TODO.org
   Added =scripts/jolo-todo.el= with state hooks:
   - =INPROGRESS= auto-creates =ID=, =SESSION_ID=, =BRANCH=, and =WORKTREE=
   - transitions out of =INPROGRESS= clock out automatically
   - =INPROGRESS= clocks in automatically
   Added =scripts/todo-agent= wrapper for multi-agent state transitions via
   =emacsclient= by heading, =ID=, or =SESSION_ID=.
