...
** TODO Generated projects should include a stub HTTP service with hot reload (including browser reload)
   ...
   - DONE TypeScript: BETH stack (Bun + Elysia + Tailwind + HTMX), =--bare= for plain Bun
...
** TODO Flatten language+variant into single flavor menu
...
** TODO Fix host ~/.tmux.conf terminal-features clobbering
   Host config has =set -g terminal-features= (non-appending) that wipes =/etc/tmux.conf=.
   Consolidate clipboard/sixel/extkeys into =set -as terminal-features ',*:clipboard:sixel:extkeys'=.
   Remove redundant =Ms= terminal-override. Must be done on host (EROFS in container).
