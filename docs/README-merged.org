...
- *Parallel agents* — Claude, Gemini, Codex in a coordinated tmux layout
- *Worktrees* — host-side (~jolo tree~) and in-container (~wt~) branching
- *Stable ports* — ~$PORT~ assigned in 4000-5000, forwarded to host, Tailscale-accessible
- *Notifications* — agents notify on completion via ntfy.sh
- *GPG signing* — agents can request signatures but cannot extract keys
- *No X11* — containers have no display access
- *Browser automation* — ~browser-check~ for ARIA snapshots, Playwright MCP for interactive flows
- *Pre-commit hooks* — language-specific linters set up automatically
- *Emacs sandbox* — your config, isolated from your host
...
This keeps agents visible and separated while still sharing the same project context. In prompt mode (~jolo up -p "..."~), the agent starts in a detached tmux session and works autonomously. When it finishes, it sends a notification.

** Worktrees

Jolo supports two tiers of workspace isolation:

- ~jolo tree~ (host-side) creates a git worktree with its own ~.devcontainer/~ and launches a full container. Use this for parallel branches, spawn runs, or risky changes that need total separation.
- ~wt~ (in-container) manages lightweight worktrees at ~/workspaces/<name>~. Use this for quick branching and sub-task exploration without the overhead of a new container.

#+begin_src bash
jolo tree feat-auth              # named worktree
jolo tree                        # random name (bold-bear, swift-falcon, ...)
...
The GPG public keyring (~pubring.kbx~, ~trustdb.gpg~) is mounted read-only. The agent socket is forwarded so agents can request signatures, but private key material stays on the host. The ~trustdb not writable~ warning is expected and harmless.
...
Every container includes ~browser-check~, a CLI tool built on Playwright with system Chromium. It provides ARIA accessibility tree snapshots — roughly 93% less context than raw HTML — so agents can inspect page structure without drowning in DOM noise.

#+begin_src bash
browser-check http://localhost:$PORT --describe          # what's on the page
browser-check http://localhost:$PORT --aria --interactive # just buttons, links, inputs
browser-check http://localhost:$PORT --screenshot         # take a screenshot
browser-check http://localhost:$PORT --console --errors   # debug JS
#+end_src

For multi-step interactive flows (clicking, filling forms, navigating), use the Playwright MCP tools which are pre-configured in each agent's settings.
...
These are included but should be considered unstable:

- *OpenSpec* (~openspec/~) — a structured artifact workflow for planning and implementing changes step by step.
- *Beads worktrees* — a secondary worktree workflow for larger multi-branch change management.
...
container/               # scripts baked into the image (tmux-layout, browser-check, wt, motd)
templates/               # scaffolds for new projects (AGENTS.md, .gitignore, skills, MCP configs)
openspec/                # structured change workflow (experimental)
