...
The Claude =SessionEnd= hook fires when the session exits. We inject it
...
- =Stop= fires every time the agent finishes a response — in interactive
  sessions this would spam a notification on every reply
- =SessionEnd= fires on session exit (completion, =ctrl-c=, =/exit=, etc.)
- In prompt mode (=-p=), the agent exits after completing the prompt, so
  =SessionEnd= fires exactly once at the right time
- Changed from =Stop= after Codex review caught the spam issue
...
def setup_notification_hooks(workspace_dir: Path) -> None:
    ...
    settings = _load_json_safe(claude_settings)
