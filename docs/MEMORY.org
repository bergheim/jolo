...
- Org task flow for this repo: =TODO -> NEXT -> INPROGRESS -> BLOCKED -> DONE=. Entering =INPROGRESS= auto-creates =ID/SESSION_ID/BRANCH/WORKTREE= and clocks in via =scripts/jolo-todo.el=; use =scripts/todo-agent= for agent-safe transitions via =emacsclient= :org:todo:worktree:clock:
