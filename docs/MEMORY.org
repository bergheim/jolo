...
- Org task flow for this repo: =TODO -> NEXT -> INPROGRESS -> BLOCKED -> DONE=. Control TODO lives in main worktree =docs/TODO.org=. Entering =INPROGRESS= auto-creates =ID/SESSION_ID/BRANCH/WORKTREE= and clocks in via =scripts/jolo-todo.el=. State changes in non-control worktree TODO files are routed to control and local state is restored to avoid divergence. Use =scripts/todo-agent= for agent-safe transitions/notes via =emacsclient= from any worktree :org:todo:worktree:clock:emacsclient:
