...
- Org task flow for this repo: =TODO -> NEXT -> INPROGRESS -> BLOCKED -> DONE=. Control TODO lives in main worktree =docs/TODO.org=. Entering =INPROGRESS= auto-creates =ID/SESSION_ID/BRANCH/WORKTREE= and clocks in via =scripts/jolo-todo.el=; use =scripts/todo-agent= for agent-safe transitions/notes via =emacsclient= from any worktree :org:todo:worktree:clock:emacsclient:
