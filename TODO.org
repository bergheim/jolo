...
* TODO Add Claude Code skills for complex multi-step workflows

Skills (slash commands) are overkill for simple =just= wrappers — AGENTS.md handles that.
But for complex multi-step workflows (deploy pipelines, database migrations, test+lint+commit
sequences), custom skills could be useful.

** Ideas
- =/deploy= — build, test, push, tag, deploy in sequence
- =/db-migrate= — run migration, verify schema, seed if needed
- =/release= — bump version, changelog, tag, push, create GH release

** Notes
- Skills live in =.claude/commands/= as markdown files
- Each skill is a prompt template that Claude executes
- Only worth adding when the workflow has branching logic or error handling
  that a simple =just= recipe can't express

* TODO Add =jolo delete= — interactive worktree/container removal

Interactive picker (like =jolo open=) to select a worktree and delete both the
worktree and its associated container.

** Requirements
- [ ] =jolo delete= lists worktrees with container status
- [ ] Reuse =_format_container_display()= for display formatting
- [ ] Interactive selection via fzf > gum > numbered fallback (same pattern as =jolo open=)
- [ ] Stop and remove the selected container
- [ ] Remove the git worktree (=git worktree remove=)
- [ ] Confirmation prompt before destructive action
- [ ] Skip picker when only one worktree exists (confirm and delete directly)

** Design decisions
- Only show worktrees (not the main repo checkout)
- Show container state in picker (running/stopped/none)
- Force-remove stopped containers, prompt if running
- If worktree has uncommitted changes, warn and require =--force= or confirmation
...
* TODO Add a justfile for the jolo meta-project

Common dev tasks for this repo (not the generated per-project justfiles).
Recipes to consider: run tests, run specific test file, lint, build container
image, install jolo symlink, etc.

* TODO Tasks, agent, skill or similar for setting up how to program

Ie. functional, prefer smaller modules and packages to one big file, etc
