...
** TODO Review /afk branches (5 local, unmerged)
   Branches from autonomous session, need review before merge or delete:
   - =yolo/export-remove-image= — export remove_image function
   - =yolo/test-container-functions= — test coverage for container.py
   - =yolo/test-reassign-port= — test coverage for reassign_port
   - =yolo/fix-entrypoint-shellcheck= — shellcheck fixes in entrypoint.sh
   - =yolo/test-toml-helper= — test coverage for TOML helpers
