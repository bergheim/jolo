...
*** Current State
=.histfile= persisted in project directory.

- [X] Added to .gitignore
- [X] Won't be committed accidentally

** OPTIONAL: Emacs Configuration

*** Current State
- =start-emacs.sh= uses a sandbox (yadm worktree) - SAFE
- =yolo.py= mounts real config writable

*** This One Is Actually Configurable
If you're using yolo.py for CLI/tmux work (not Emacs), you could:
- Mount emacs config readonly
- Or not mount it at all

This is the one place where there's a reasonable security/usability tradeoff.

*** Potential Risk
If emacs config is writable and you also use Emacs *outside* the container, malicious elisp could persist.

*** Status
- [X] Made emacs config readonly in yolo.py
- [ ] Consider: separate cache dir for container (lower priority)
- Only matters if you use Emacs both inside and outside container
...
- [X] Current setup is fine for directed use (you're watching)
- [ ] Optional: make ~/.config/emacs readonly if you don't edit Emacs config in container
