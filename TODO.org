...
** TODO Add `jolo research` command with async report delivery (email or ntfy)
   Use case: run research prompt, return immediately, send results on completion.
** TODO =wt new= should only create 1 tmux window (shell), not the full 5-window layout
