...
* TODO Smart MOTD on every new shell session
...
* TODO Prefix container/image names with project name

Random names like =busy-badger= don't indicate which project they belong to.
Container names should be prefixed with the project name, e.g. =myapp-busy-badger=.
This makes =podman ps= / =jolo list --all= output immediately meaningful.

* TODO Add interactive project switcher to jolo

List all active jolo projects and let the user select one to switch to.

** Requirements
- [ ] =jolo --switch= (or similar) lists all active/running jolo projects
- [ ] Display useful info per project (name, status, worktree, port, container state)
- [ ] Interactive selection to switch into the chosen project's container
- [ ] Reuse existing =--list= discovery logic for finding active projects
