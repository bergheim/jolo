...
** TODO Tighten BETH TypeScript scaffold plan before implementation
   Blockers from =openspec/changes/beth-stack-scaffold/plan.md= review:
   - Require =if (import.meta.main)= listen gate + exported =app= for tests
   - Add explicit =@kitajs/html= dependency when using =jsxImportSource=
   - Avoid coupling app source scaffolding to =get_test_framework_config=
