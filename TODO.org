...
* TODO Mount ~/.claude RW instead of copying credentials
:PROPERTIES:
...
* TODO Worktree-in-container quick mode (no host visibility)

Idea: allow =just tree foo= to create an *ephemeral* worktree inside the running
container (not mounted on host). Fast, but it disappears on =jolo down=.

Tradeoffs:
- Pros: instant, no new container, easy scratch work
- Cons: not host-visible, must merge/export before shutdown, less isolation control
...
* TODO Cache zoxide/mise init scripts                      :zsh:perf:
:PROPERTIES:
:DATE: 2026-02-09
:END:

~eval "$(zoxide init zsh)"~ and ~eval "$(mise activate zsh)"~ run on every
shell startup, generating and evaling shell code each time. Cache the output
to a file and source it instead. Regenerate when the tool version changes.
...
* TODO Run motd only in the shell window                   :tmux:perf:
:PROPERTIES:
:DATE: 2026-02-09
:END:
...
* TODO Agent completion notifications via ntfy.sh          :agents:notifications:
:PROPERTIES:
:DATE: 2026-02-09
:END:
