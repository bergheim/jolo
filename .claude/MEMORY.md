...
- 2026-04-13/14: Big docs+infra session. Wayland Chromium in containers: renders but crashes (seccomp blocks sched_getscheduler) and input broken — cancelled. Org-mode emacsclient workflow discovered: org-todo + org-add-log-note + org-store-log-note for proper CLOSED timestamps and LOGBOOK. org-element-map breaks logging hooks — use re-search-forward instead. Flavor rename -bare→plain. pa11y for a11y testing (axe-core CLI broken on Alpine — uses Selenium). tmux multi-client (drop -d, window-size largest). Stale skill cleanup in sync. PG stale PID + comma-separated socket dirs fixed. Plugins RW mount. AVIF/WebP image guidance. Skip-nav in scaffolds. PUPPETEER_EXECUTABLE_PATH in containerEnv. UV_LINK_MODE warning on Podman.
- 2026-04-16/17: Denote Phase 2 — implemented bergheim/agent-denote-link using denote.el APIs, cross-linked all 15 notes, added .dir-locals.el ("." trick avoids trust prompts). Code review caught: missing lexical-binding, regex escape bug in collision handler (^#+identifier matched "one or more #"), side-effecting mapcar, wrong return value. agent-helpers.el copied to /workspaces/stash/ and loaded from host config. OSC 52 interprogram-cut-function was breaking X11 clipboard — needs EMACS_CONTAINER guard (host config fix). Merged feat/agent-denote to main.
