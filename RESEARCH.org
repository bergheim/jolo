...
=SessionEnd= runs when a session ends. citeturn0search1

** Codex CLI (confirmed: notify hook)

Codex CLI supports a =notify= setting in =config.toml=. The configuration
reference states that =notify= is an array of strings (command argv) and that
the command is invoked for notifications, receiving a JSON payload from Codex.
This is the supported mechanism for “after turn” notifications. citeturn2view0turn3view0

** Gemini CLI (unconfirmed)

No official hooks documentation was found in primary sources. There are GitHub
issues from users describing =hooks= and =enableHooks= in =~/.gemini/settings.json=,
but these are not authoritative docs. Treat Gemini hooks as unconfirmed until
official documentation is located. citeturn0search0
