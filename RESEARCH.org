...
* Junk drawer shared mount for devcontainers           :jolo:devcontainer:mounts:
:PROPERTIES:
:DATE: 2026-02-11
:END:

Added a default RW shared mount intended as a non-reproducible "junk drawer"
for ad-hoc notes and scratch files.

** Decision
- Host path: ~/jolo
- Container path: /workspaces/junk-drawer
...
- MOTD banner includes junk drawer warning

** Key files
- ~_jolo/constants.py~ (BASE_MOUNTS)
- ~_jolo/setup.py~ (setup_junk_drawer)
- ~_jolo/commands.py~ (call setup_junk_drawer)
- ~container/motd~ (junk drawer warning)
