...
Alpine Linux container with Emacs, agents (Claude, Gemini, Codex, Pi) without permission checks, language servers, PostgreSQL + pgvector, browser automation, lots of guard rails (linters, pre-commit hooks), and all the developer tools I've come to expect on regular systems.
