...
All agents share the same skills and project memory (~docs/~). Skills are namespaced with ~j-~ for quick invocation (e.g. ~/j-next~, ~/j-review~). Agents get push notifications via ntfy when they finish, with buttons that link to the commit, PR, or the running app depending on context.
