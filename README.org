...
~just wt~ creates branches as worktrees under ~/workspaces/<name>/~ with their own tmux session. Worktrees share refs with main, so landing branches back doesn't need a fetch/push round-trip.

| Command                          | Effect                                          |
|----------------------------------+-------------------------------------------------|
| ~just wt new <name>~             | Create branch + worktree + tmux session         |
...
| ~just wt rm [<name>]~            | Remove worktree + branch + tmux session         |
