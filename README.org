...
This repo includes =scripts/jolo-todo.el= for =docs/TODO.org= workflow
automation with states:

#+begin_example
TODO -> NEXT -> INPROGRESS -> BLOCKED -> DONE
#+end_example

Control-plane behavior: all updates target the main worktree's
=docs/TODO.org= automatically, even when called from another worktree.
If you change TODO state in a non-control worktree file (for example with
=C-c C-t=), it is routed to the control file and local state is restored.

When a task moves to =INPROGRESS=, it auto-creates =ID=, =SESSION_ID=,
=BRANCH=, and =WORKTREE= properties, creates a git worktree (prefers
=/workspaces/= when writable, else uses =worktrees/= under the repo), and
clocks in. Leaving =INPROGRESS= clocks out.

Load it in Emacs:

#+begin_src emacs-lisp
(let ((root (locate-dominating-file default-directory "AGENTS.md")))
  (load-file (expand-file-name "scripts/jolo-todo.el" root))
  (jolo-todo-enable))
#+end_src

Emacs keybindings (org-mode):

#+begin_example
C-c j o  open control TODO
C-c j n  NEXT
C-c j s  INPROGRESS
C-c j b  BLOCKED
C-c j d  DONE
C-c j m  add note
C-c j r  resume by SESSION_ID
#+end_example

Agent-safe transitions from shell:

#+begin_src bash
scripts/todo-agent next --headline "Investigate network egress monitoring for devcontainers"
scripts/todo-agent start --session 20260307T153000Z-ab12cd
scripts/todo-agent block --id 9a5fbe41-6a74-4ceb-8f60-ae5dd7f3010f
scripts/todo-agent done --headline "Add ntfy desktop notifications on Linux"
scripts/todo-agent note --session 20260307T153000Z-ab12cd --message "implemented worktree auto-detection"
#+end_src
