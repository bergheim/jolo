...
~just wt~ creates branches as worktrees under ~/workspaces/<name>/~ as windows (~wt:<name>~) in the current tmux session. Worktrees share refs with main, so landing branches back doesn't need a fetch/push round-trip.
