...
class TestStatusPreviewLine(unittest.TestCase):
    def test_public_url_is_reported_when_published(self):
        ...
        routes = {"test4k.dev.glvortex.net": (4676, "$2a$14$h")}
        with mock.patch.object(
            commands.sites, "is_available", return_value=True
        ):
            with mock.patch.object(
                commands.sites, "read_previews", return_value=routes
            ):
                line = commands._preview_status_line("test4k")

        self.assertEqual(
            line, "Preview: https://test4k.dev.glvortex.net (auth)"
        )

    def test_open_publication_is_flagged(self):
        from _jolo import commands

        routes = {"test4k.dev.glvortex.net": (4676, None)}
        with mock.patch.object(
            commands.sites, "is_available", return_value=True
        ):
            with mock.patch.object(
        ...
        self.assertEqual(
            line, "Preview: https://test4k.dev.glvortex.net (NO AUTH)"
        )
    ...
    def test_nothing_reported_when_not_published(self):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(
                commands.sites, "read_previews", return_value={}
            ):
                self.assertIsNone(commands._preview_status_line("test4k"))
